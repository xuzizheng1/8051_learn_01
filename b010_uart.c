...
接口说明：　　
	...
	4. 在发送缓冲区中输入字符，如：42
	5. 点击发送数据
	6. 接收缓冲区中将显示发送->42，接收->09 02，因为我们将a的值分别赋值为9和2发送，可以用于调试程序的变量值
修改日志：　　
　　							
/*********************************************************************************************
说明：
...
#include <string.h>
#include <STC12C5A60S2.h>	// STC12C5A60S2 单片机头文件
//#include <STC12C2052AD.H> //STC12Cx052或STC12Cx052AD系列单片机头文件


sbit P00 = P0 ^ 0; // led正极接Vcc，负极接P00引脚
...
void main (void){
	...
	P00 = 1; // 初始led熄灭
	UART_init();
	UART_TC("Input 3 to control the led:\n");
	while(1){
		if (RI == 1){		//接收中断标志位为1时
			UART_data = SBUF;	//接收数据 SBUF 为单片机的接收发送缓冲寄存器
			if(UART_data==0x30 + 3){
				P00 = ~P00; // 当按键盘3键时，控制led亮灭交替
			}
